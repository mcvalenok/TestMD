...
Взаимодействие сторонних приложений с сервисом электронных платежей осуществляется по HTTP протоколу. Данные запроса и его результат передаются в JSON.
...
*Метод позволяет создать платеж в сервисе электронных платежей.*
...
*API* — Application Programming Interface. Набор методов (функций), который программист может использовать для доступа к функциональности программного компонента (программы, модуля, библиотеки).

*REST* — Representational State Transfer. Набор правил, по которым осуществляется обмен данными между различными приложениями сети. 

*HTTP* — Hyper Text Transfer Protocol. Протокол передачи гипертекста. Является протоколом прикладного уровня стека TCP/IP.
